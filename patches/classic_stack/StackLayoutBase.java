...
import org.chromium.chrome.browser.compositor.layouts.eventfilter.MotionEventFilter;
...
import org.chromium.chrome.browser.compositor.scene_layer.ClassicStackSceneLayer;
...
public abstract class StackLayoutBase extends Layout {
    ...
    private final MotionEventFilter mGestureEventFilter;
    ...
    private ClassicStackSceneLayer mSceneLayer;
    private boolean mShowPending;

    private class StackLayoutGestureHandler implements MotionEventHandler {
        @Override
        ...
        // The following four methods did not exist on the old GestureHandler
        // interface -- MotionEventHandler added them for mouse/desktop support.
        // TODO(aerium): implement real hover feedback for the stack if desired;
        // no-ops preserve original touch-only M88 behavior.
        @Override
    ...
    public StackLayoutBase(Context context, LayoutUpdateHost updateHost,
        ...
        // TODO(aerium): verify autoOffset/useDefaultLongPress against real device
        // behavior -- these two args did not exist on the old GestureEventFilter
        // constructor. autoOffset=false assumes Stack positions events itself
        // without needing EventFilter's built-in motion offset tracking (matches
        // old behavior, where GestureEventFilter had no offset concept either).
        // useDefaultLongPress=true keeps standard Android long-press timing/
        // cancellation semantics, closest to the old default.
        mGestureEventFilter =
                new MotionEventFilter(
                        context,
                        mGestureHandler,
                        /* autoOffset= */ false,
                        /* useDefaultLongPress= */ true);
    ...
    public void simulateClick(float x, float y) {
        mGestureHandler.click(x, y, /* buttons= */ -1, /* modifiers= */ 0);
    }
    ...
    public void simulateDrag(float x, float y, float dX, float dY) {
        mGestureHandler.onDown(x, y, /* buttons= */ -1);
    ...
    private void ensureSceneLayerCreated() {
        ...
        mSceneLayer = new ClassicStackSceneLayer();
    }

    // TODO(aerium): CONFIRMED CHANGE, NOT YET RESOLVED -- the current Layout base
    // class's updateSceneLayer() no longer takes a LayerTitleCache parameter at
    // all (verified against the live ToolbarSwipeLayout.java override, which has
    // the 5-arg shape below with no LayerTitleCache). The old TabListSceneLayer
    // .pushLayers() used LayerTitleCache for per-card title/favicon compositing
    // during transitions. Since TabListSceneLayer itself no longer exists (this
    // whole class is being rewritten as ClassicStackSceneLayer, modeled on
    // TabStripSceneLayer's write-index/reuse pattern -- see chat), decide there
    // whether ClassicStackSceneLayer needs LayerTitleCache at all, and if so,
    // obtain it some other way (e.g. as a constructor arg to StackLayoutBase, or
    // a field populated some other current-API path) rather than assuming this
    // base-class parameter still exists.
    @Override
    protected void updateSceneLayer(RectF viewport, RectF contentViewport,
        ...
        // TODO(aerium): pushLayers(...) was TabListSceneLayer's own Java API, not
        // part of any Chromium-wide contract -- it disappears along with that
        // class. Replace this call with whatever method ClassicStackSceneLayer
        // ends up exposing (likely a beginBuildingFrame() / per-tab put.../
        // finishBuildingFrame() sequence, mirroring TabStripSceneLayer's
        // pushAndUpdateStrip() shape -- see chat for the confirmed native
        // write-index/reuse pattern this should be built on).
