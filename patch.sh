...
# ==============================================================================
# [26] AERIUM CLASSIC 3D OVERLAPPING STACK TAB SWITCHER (CONNECTED TO PATCHES/)
# ==============================================================================
echo "==> [26] Wiring patches/classic_stack into LayoutManagerChromePhone & Settings..."
...
TARGET_STACK="$TARGET_PHONE/stack"
TARGET_SCENE="chrome/android/java/src/org/chromium/chrome/browser/compositor/scene_layer"

mkdir -p "$TARGET_STACK"
mkdir -p "$TARGET_SCENE"

if [ -d "$PATCHES_DIR" ]; then
    cp "$PATCHES_DIR/StackLayoutBase.java" "$TARGET_PHONE/" 2>/dev/null || true
    cp "$PATCHES_DIR/StackLayout.java" "$TARGET_PHONE/" 2>/dev/null || true
    cp "$PATCHES_DIR/Stack.java" "$TARGET_STACK/" 2>/dev/null || true
    cp "$PATCHES_DIR/StackTab.java" "$TARGET_STACK/" 2>/dev/null || true
    cp "$PATCHES_DIR/StackAnimation.java" "$TARGET_STACK/" 2>/dev/null || true
    cp "$PATCHES_DIR/OverlappingStack.java" "$TARGET_STACK/" 2>/dev/null || true
    cp "$PATCHES_DIR/ClassicStackSceneLayer.java" "$TARGET_SCENE/" 2>/dev/null || true
    echo "[aerium] Successfully copied 7 Classic Stack files from $PATCHES_DIR"
else
    echo "[FATAL] patches/classic_stack not found!"
    exit 1
fi
...
# --- 2. Register all 7 files in chrome_java_sources.gni ---
...
for src in new_sources:
    raw = src.strip('",')
    if raw not in gni_c:
        gni_c = gni_c.replace("chrome_java_sources = [", "chrome_java_sources = [\n  " + src, 1)

with open(gni_path, "w", encoding="utf-8") as f:
    f.write(gni_c)
print("[aerium] Registered 7 classic stack files in chrome_java_sources.gni")

# --- 3. Inject Strings into android_chrome_strings.grd ---
...
# --- 4. Inject Arrays uniquely into chrome/android/java/res/values/values.xml ---
target_res_xml = None
...
# --- 5. Inject Settings into tabs_settings.xml ---
...
"""
    ...
    with open(settings_path, "w", encoding="utf-8") as f:
        f.write(set_c)
    print("[aerium] Injected preference into tabs_settings.xml")

# --- 6. TabUiFeatureUtilities helpers ---
...
"""
    ...
    if idx != -1:
        ...
        with open(util_path, "w", encoding="utf-8") as f:
            f.write(u_c)
        print("[aerium] Injected helpers into TabUiFeatureUtilities")

# --- 7. Wire StackLayout into LayoutManagerChromePhone.java ---
lm_path = find_file("LayoutManagerChromePhone.java", path_hint=os.path.join("compositor", "layouts"))
with open(lm_path, "r", encoding="utf-8") as f:
    lm_c = f.read()

# Add import if needed
if "org.chromium.chrome.browser.compositor.layouts.phone.StackLayout;" not in lm_c:
    lm_c = "import org.chromium.chrome.browser.compositor.layouts.phone.StackLayout;\n" + lm_c

if "mStackLayout" not in lm_c:
    # Add field
    lm_c = re.sub(
        r'(public class LayoutManagerChromePhone extends [^{]*\{)',
        r'\1\n    private StackLayout mStackLayout;',
        lm_c,
        count=1
    )
    # Instantiate in initLayouts
    instantiation = """
        if (TabUiFeatureUtilities.isVerticalStackSelected()) {
            mStackLayout = new StackLayout(context, this, mRenderHost, mBrowserControlsStateProviderSupplier);
            addSceneOverlay(mStackLayout);
        }
"""
    if "initLayouts" in lm_c:
        lm_c = re.sub(r'(protected void initLayouts\([^{]*\{)', r'\1' + instantiation, lm_c, count=1)
    
    with open(lm_path, "w", encoding="utf-8") as f:
        f.write(lm_c)
    print("[aerium] Wired StackLayout into LayoutManagerChromePhone.java")
