...
# --- 2. RESTORE TABLISTCOORDINATOR.JAVA TO PRISTINE GIT STATE ---
coord_path = find_file("TabListCoordinator.java", path_hint=os.path.join("tasks", "tab_management"))

# Use git checkout to undo any truncated lines
try:
    subprocess.run(["git", "checkout", "--", coord_path], check=False)
except Exception:
    pass
...
# Safety check: if methods were truncated, restore using git archive or checkout
if "getContainerView()" not in c:
    print("[aerium] TabListCoordinator was truncated, forcing checkout...")
    subprocess.run(["git", "checkout", "HEAD", "--", coord_path], check=False)
    with open(coord_path, "r", encoding="utf-8") as f:
        c = f.read()
...
# --- 4. Step C: Preference Arrays injection (Chrome's values.xml only) ---
target_res_xml = None
...
# --- 8. Step G: TabListCoordinator Integration (NO TRUNCATION, NO RETURN;) ---
with open(coord_path, "r", encoding="utf-8") as f:
    coord_c = f.read()
...
# 3. setDefaultGridCardSize: Pass custom size WITHOUT return;
...
with open(coord_path, "w", encoding="utf-8") as f:
    f.write(coord_c)
print("[aerium] TabListCoordinator cleanly integrated without touching other methods")

# --- 9. Step H: TabListMediator (1 card per row for vertical stack) ---
