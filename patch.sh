...
# ==============================================================================
echo "==> [26] Running patches/classic_stack_patch.py..."
...
# Find the Python script reliably across paths
PATCH_SCRIPT=""
for candidate in \
    "$GITHUB_WORKSPACE/patches/classic_stack_patch.py" \
    "$(pwd)/patches/classic_stack_patch.py" \
    "$(pwd)/../patches/classic_stack_patch.py" \
    "$(pwd)/../../patches/classic_stack_patch.py"; do
    if [ -f "$candidate" ]; then
...
if [ -z "$PATCH_SCRIPT" ] || [ ! -f "$PATCH_SCRIPT" ]; then
    PATCH_SCRIPT=$(find /home/runner/work/ -maxdepth 5 -type f -name "classic_stack_patch.py" 2>/dev/null | head -n 1)
fi

if [ -f "$PATCH_SCRIPT" ]; then
    echo "[aerium] Executing $PATCH_SCRIPT..."
    python3 "$PATCH_SCRIPT"
else
    echo "[FATAL] patches/classic_stack_patch.py not found!"
    exit 1
fi
